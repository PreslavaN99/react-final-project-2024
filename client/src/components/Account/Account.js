...
const Account = () => {
    ...
    function deleteFavAnimal({username, id}) {
        ...
        deleteFavAnimalByUsernameAndId(username, id)
            .then(res => res.json())
            .then(data => {
                let filteredFavAnimals = favouritesAnimal.filter(x => x.id !== data.AnimalId);
    ...
    return (
        <div className={'wrap-main'}>
            ...
            <div className={'wrap-commented-Animals'}>
                ...
                <section className={'commented-Animals'}>
                    {commentedAnimals.length ?
                        commentedAnimals.map((x, id) => <CommentedAnimal key={++id} Animal={x}/>)
                        :
            ...
            <div className={'wrap-cards fav'}>
                ...
                {favouritesAnimal.length ? favouritesAnimal.map(x => <AnimalFav key={x.id} Animal={x} username={username}
